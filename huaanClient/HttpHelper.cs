...
namespace huaanClient
{
    class HttpHelper
    {
        ...
        private void SetCer(HttpItem item)
        {
            if (!string.IsNullOrWhiteSpace(item.CerPath))
            {
                ...
                SetCerList(item);
                //将证书添加到请求里
                if (!string.IsNullOrWhiteSpace(item.CerPathPassWord))
                {
                    request.ClientCertificates.Add(new X509Certificate(item.CerPath, item.CerPathPassWord, X509KeyStorageFlags.MachineKeySet));
                }
                else
                {
        ...
        private void SetProxy(HttpItem item)
        {
            ...
            {
                request.Proxy = new WebProxy(item.ProxyIp);
                if (!string.IsNullOrEmpty(item.ProxyUserName) && !string.IsNullOrEmpty(item.ProxyPwd))
                {
                    request.Proxy.Credentials = new NetworkCredential(item.ProxyUserName, item.ProxyPwd);
                }
            }
        }
    ...
    public class HttpItem
    {
        ...
        /// 证书密码
        /// </summary>
        public string CerPathPassWord { get; set; }
        /// <summary>
